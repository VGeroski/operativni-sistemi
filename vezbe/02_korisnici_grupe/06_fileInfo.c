/**
 * Program treba da ispise informacije o fajlu po uzoru na 'ls -al'
 */
...
/**
 * Funkcija cita korisnikovo ime iz passwd fajla na osnovu numerickog id-a korisnika
 */
char *os_get_user_info_uid(uid_t uid)
{
    /* citamo informacije iz fajla na osnovu numerickog id-a korisnika */
    ...
    /* kopiramo korisnicko ime u lokalni adresni prostor */
...
/**
 * Funkcija cita ime grupe iz group fajla na osnovu numerickog id-a grupe
 */
char *os_get_group_info_gid(gid_t gid)
{
    /* citamo informacije iz fajla na osnovu numerickog id-a grupe */
...
char *file_information(const char *filePath)
{
    ...
    /* inicijalno podrazumevamo da nad fajlom postoje sva prava pristupa */
     ...
     * naredni upis
     */
    offset += sprintf(fileInfoStr + offset, "%ju ", fInfo.st_nlink);

    /* iz passwd fajla citamo ime korisnika na osnovu njegovog numerickog id-a */
    ...
    /* iz group fajla citamo ime grupe na osnovu njenog numerickog id-a */
    ...
    offset += sprintf(fileInfoStr + offset, "%jd ", fInfo.st_size);
